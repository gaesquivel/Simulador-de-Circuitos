...
{
    public class DCSolverH1: CircuitSolver
    {
        Circuit circuit;
        ...
        protected static void Calculate(List<Node> nodosnorton)
        {
            ...
            if (nodosnorton.Count > 0)
            {
                ...
                var v = Vector<Complex>.Build.Dense(nodosnorton.Count);
                var A = Matrix<Complex>.Build.DenseOfArray(new Complex[nodosnorton.Count, nodosnorton.Count]);
        ...
        /// <param name="v1"></param>
        /// <param name="z1"></param>
        private static void NavigateBranch(Node nodo, Dipole compo1, ref Complex v1, ref Complex z1)
        {
            Node nodo1 = nodo;
